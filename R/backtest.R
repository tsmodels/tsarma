...
#' @param object an object of class \\dQuote{tsarma.spec}.
...
#'
#'
tsbacktest.tsarma.spec <- function(object, start = floor(length(object$target$y_orig))/2,
                                    ...
                                    estimate_every = 1, rolling = FALSE, trace = FALSE, ...)
{
    ...
    b %<-% future_lapply(1:length(seqdates), function(i) {
        ...
        spec <- arma_modelspec(y_train, constant = object$model$constant, order = object$model$order,
                                xreg = xreg_train, distribution = object$distribution)
        ...
        # xreg
        if (use_xreg) {
            xreg_test <- xreg[paste0(seqdates[i],"/",M[[1]]$forecast_dates)]
        } else {
            xreg_test <- NULL
        }
        # actual
        P <- vector(mode = "list", length = rolls)
        tmp <- predict(mod, h = M[[1]]$h[1], newxreg = xreg_test, forc_dates = M[[1]]$forecast_dates)
        if (rolls > 1 & rolling) {
            ...
            for (j in 2:rolls) {
                ...
                tmp <- predict(f, h = M[[j]]$h[1], newxreg = xreg_test, forc_dates = M[[j]]$forecast_dates)
        ...
        return(out)
    }, future.packages = c("tsmethods","tsarma","xts","data.table"), future.seed = TRUE)
    b <- eval(b)
    b <- rbindlist(b)
