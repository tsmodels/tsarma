...
#' @method estfun tsarma.estimate
#' @aliases estfun
#' @rdname estfun
#' @author Alexios Galanos
#' @export
#'
estfun.tsarma.estimate <- function(x, ...)
{
    out <- x$scaled_scores
    N <- nrow(out)
...
#' @method bread tsarma.estimate
#' @aliases bread
#' @rdname bread
#' @author Alexios Galanos
#' @export
#'
bread.tsarma.estimate <- function(x, ...)
{
...
meat_tsarma <- function(x, adjust = FALSE, ...)
{
    psi <- estfun(x, ...)
    k <- NCOL(psi)
    n <- NROW(psi)
...
meatHAC_tsarma <- function(x, prewhite = FALSE, weights = NULL,  lag = NULL,
